    ...
    private void btnOkActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOkActionPerformed
        if (grSelect.getRowCount() != 0) {
            if (form.equals("recep")) {
                if (from == 1) {
                    DatPhong.it.tempRecep.setRoomID((int) grSelect.getValueAt(0, 2));
                }
                DatPhong.it.tempRecep.setExpectedCheckoutDate(dcsTo.getDate());
                DatPhong.it.fillData(DatPhong.it.tempRecep);
                this.dispose();
            }
        } else {
